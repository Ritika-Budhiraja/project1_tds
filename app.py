# app.py
from fastapi import FastAPI, HTTPException, File, UploadFile, Form, Body, Header, Depends
...
FALLBACK_TO_AI = True  # Toggle for AI fallback feature
...
app = FastAPI(title="RAG Query API", description="API for querying the RAG knowledge base", dependencies=[Depends(verify_api_key)])
...
async def query_knowledge_base(request: QueryRequest):
    try:
        ...
        relevant_results = await find_similar_content(question_embedding, conn)
        
        # If no relevant results found and AI fallback is enabled
        if not relevant_results and FALLBACK_TO_AI and configure_gemini():
            logger.info("No relevant results found, falling back to Gemini AI")
            try:
                ...
                If you're not sure, say "I'm not confident about this answer."
                """
                ...
                return QueryResponse(
                    ...
                    links=[LinkInfo(
                        ...
                        text="Generated by AI (no direct source available)"
                    )]
                )
            except Exception as e:
                logger.error(f"Error using Gemini AI fallback: {e}")
                raise HTTPException(status_code=500, detail="Error generating AI response")
        
        # Generate answer from relevant results
        answer = await generate_answer(request.question, relevant_results)
        
        # Parse response and extract sources
        parsed_response = parse_llm_response(answer)
